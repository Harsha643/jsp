...
loading.style.display="block"
 ...
 if(data==[]){
  ...
  nodata.innerHTML=
  `
  <h1>Draft data is not present</h1>
  
  `
 }
}

let homed=document.getElementById("home")
...
async function deleteData(id) {
  try {
    let response = await fetch(`https://amplified-hulking-snowdrop.glitch.me/meals/${id}`, {
      method: "DELETE",
    });
    if (!response.ok) {
...
async function saveData() {
  ...
  try {
    ...
    let url = recipeId
      ? `https://amplified-hulking-snowdrop.glitch.me/meals/${recipeId}`
